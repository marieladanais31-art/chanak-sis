...
const DeclarationOfEnrolment = ({ studentId, paymentStatus, studentName, studentLastName, gradeLevel, passportNumber, isScholarship }) => {
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      const p1 = `This letter confirms that the following student is duly enrolled and registered with CHANAK TRAINUP EDUCATION INC (operating as Chanak International Academy) for the academic year ${currentYear}-${currentYear + 1}:`;
      ...
      // Body Paragraph 2 - IEP and Distance Learning
      doc.setFontSize(12);
      doc.setFont('times', 'normal');
      const p2 = `The student is actively following an Individualized Educational Plan (IEP) based on the Accelerated Christian Education (A.C.E.) curriculum, utilizing our recognized distance learning model to fulfill all educational requirements in compliance with international standards.`;
      const lines2 = doc.splitTextToSize(p2, 170);
      doc.text(lines2, 20, 170);

      const p3 = `Chanak International Academy is a private international educational institution dedicated to providing individualized academic excellence.`;
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col space-y-5 h-full">
      <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Carta Formal de Matrícula</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
        {!canDownload ? (
          <div className="bg-amber-50 border border-amber-200 p-4 rounded-xl text-sm text-amber-800 w-full">
            La carta de matrícula estará disponible una vez confirmado el pago.
          </div>
        ) : (
...
export default DeclarationOfEnrolment;
