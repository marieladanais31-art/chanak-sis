...
export default function DeclarationOfEnrolmentPDF({ studentName, passportNumber, gradeLevel }) {
  ...
  const generatePDF = () => {
    ...
    try {
      ...
      toast({ title: 'Éxito', description: 'Declaración generada correctamente.' });
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col space-y-4">
      <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
        ...
        <div>
          <h3 className="font-bold text-slate-800">Declaration of Enrolment</h3>
          <p className="text-xs text-slate-500">Documento Oficial de Matrícula</p>
        </div>
      </div>
      
      <div className="bg-slate-50 p-4 rounded-xl text-sm text-slate-600 flex items-start gap-3">
        <CheckCircle2 className="w-5 h-5 text-emerald-500 shrink-0" />
        <p>Certifica la inscripción y el Plan Educativo Individualizado activo bajo lineamientos internacionales.</p>
      </div>
