...
import { computeAverage, syncStudentSubjectGrade } from '@/lib/academicRecords';
import { getRecommendedMinimum } from '@/lib/academicUtils';

export default function GradeEntriesManager({ studentSubject, canEdit = false, onEntriesChange }) {
  ...
  const loadEntries = async () => {
    ...
    try {
      ...
      const nextAverage = await syncStudentSubjectGrade(supabase, studentSubject.id, nextEntries);
      onEntriesChange?.(nextAverage, nextEntries);
    } catch (err) {
  ...
  const average = useMemo(() => computeAverage(entries), [entries]);
  ...
  const addEntry = async () => {
    ...
    try {
      const maxOrder = entries.length > 0 ? Math.max(...entries.map(e => e.entry_order || 0)) : 0;
      
      const newEntry = {
        student_subject_id: studentSubject.id,
        student_id: studentSubject.student_id,
        quarter: studentSubject.quarter || 'Q1',
        school_year: studentSubject.school_year || '2025-2026',
  ...
  const updateEntry = async (id, field, value) => {
    ...
    try {
      ...
      // If changing score, make sure it's a number
      if (field === 'score') {
        updateData.score = value === '' ? 0 : parseFloat(value);
      }
      ...
      const nextEntries = previousEntries.map(e => e.id === id ? { ...e, [field]: updateData[field] } : e);
      const nextAverage = await syncStudentSubjectGrade(supabase, studentSubject.id, nextEntries);
      onEntriesChange?.(nextAverage, nextEntries);
    } catch (err) {
      console.error('Error updating entry:', err);
      setEntries(previousEntries); // Revert on error
  ...
  const deleteEntry = async (id) => {
    ...
    try {
      ...
      if (error) throw error;
      const nextEntries = previousEntries.filter(e => e.id !== id);
      const nextAverage = await syncStudentSubjectGrade(supabase, studentSubject.id, nextEntries);
      onEntriesChange?.(nextAverage, nextEntries);
      ...
      <div className="p-4 flex-1 overflow-y-auto min-h-[250px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center p-6 text-slate-500">
            ...
            <p className="text-sm">Agregue notas parciales para calcular el promedio.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {entries.map((entry, index) => (
              <div key={entry.id} className="flex flex-col sm:flex-row gap-3 items-start sm:items-center bg-white p-3 rounded-lg border border-slate-200 shadow-sm hover:border-[#193D6D]/30 transition-colors">
                ...
                <div className="flex-1 w-full grid grid-cols-1 sm:grid-cols-12 gap-3">
                  ...
                  <div className="sm:col-span-3">
                    ...
                    {canEdit ? (
                      ...
                      <span className="text-sm text-slate-600">{entry.date_recorded ? new Date(entry.date_recorded).toLocaleDateString() : '-'}</span>
                    )}
                  </div>
                </div>
