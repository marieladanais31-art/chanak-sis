...
export default function HubSelectorClean({ selectedHubIds = [], onChange, multiple = true }) {
  ...
  useEffect(() => {
    const fetchHubs = async () => {
      console.log('HubSelectorClean: Fetching hubs...');
      try {
        ...
        console.log('HubSelectorClean: Fetched hubs:', data);
        setHubs(data || []);
      } catch (err) {
        console.error('HubSelectorClean: Error fetching hubs:', err);
  ...
  if (loading) {
    return (
      <div className="p-4 flex items-center justify-center gap-3 text-sm text-slate-500 bg-slate-50 rounded-xl border border-slate-200">
        <Loader2 className="animate-spin w-5 h-5 text-indigo-500" /> Cargando hubs dinámicamente...
      </div>
    );
  }
  ...
  if (hubs.length === 0) {
    return (
      <div className="p-6 text-center text-sm text-slate-600 bg-slate-50 rounded-xl border border-slate-200">
        ...
        <p className="text-xs mt-1">Asegúrese de que existan organizaciones con type='hub'.</p>
      </div>
    );
  }
          ...
          >
            ...
            <div className="flex flex-col flex-1 min-w-0">
              <span className="text-sm font-bold text-slate-800 flex items-center gap-1.5 truncate">
                ...
                <span className="truncate">{hub.name} {hub.code ? `(${hub.code})` : ''}</span>
              </span>
              {hub.location && (
