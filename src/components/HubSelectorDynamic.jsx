...
export default function HubSelectorDynamic({ selectedHubIds = [], onChange, multiple = true }) {
  ...
  useEffect(() => {
    const fetchHubs = async () => {
      try {
        ...
        console.error('Error fetching hubs dynamically:', err);
  ...
  if (loading) {
    return (
      <div className="p-4 flex items-center justify-center gap-3 text-sm text-slate-500 bg-slate-50 rounded-xl border border-slate-200">
        <span className="animate-spin text-xl">⏳</span> Cargando hubs dinámicamente...
      </div>
    );
  }
  ...
  if (hubs.length === 0) {
    return (
      <div className="p-6 text-center text-sm text-slate-600 bg-slate-50 rounded-xl border border-slate-200">
        ...
        <p className="font-bold">No hay Hubs registrados</p>
