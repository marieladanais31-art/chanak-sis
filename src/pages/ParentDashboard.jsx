...
export default function ParentDashboard() {
  ...
  const getChildSubjects = (childId, quarter = activeQuarter) => {
    const raw = studentSubjects.filter(
      (s) =>
        s.student_id === childId &&
        s.school_year === ACADEMIC_YEAR &&
        s.quarter === quarter
    );

    return dedupeStudentSubjects(raw).sort((a, b) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setStudentSubjects(dedupeStudentSubjects(studentSubjectsRes.data || []));
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        {children.length === 0 ? (
          ...
          <>
            {activeTab === 'children' && (
              <>
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-wrap items-center gap-2 mb-6">
                  <span className="text-sm font-black text-slate-700 uppercase tracking-wider">Trimestre activo:</span>
                  {QUARTERS.map((quarter) => (
                    <button
                      key={quarter.id}
                      onClick={() => setActiveQuarter(quarter.id)}
                      className={`px-3 py-2 rounded-lg text-sm font-bold border transition-colors ${activeQuarter === quarter.id ? 'bg-[#193D6D] text-white border-[#193D6D]' : 'bg-white text-slate-600 border-slate-200 hover:border-[#193D6D] hover:text-[#193D6D]'}`}
                    >
                      {quarter.id}
                    </button>
                  ))}
      ...
      {isBulletinModalOpen && (() => {
        ...
        const sortedSubjects = getChildSubjects(selectedChildId, activeQuarter);
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            ...
            <div id="printable-bulletin" className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col relative print:bg-white print:max-h-none print:w-full">
              <div className="p-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center print-hide">
                <h3 className="font-black text-lg text-[#193D6D]">{`Vista Previa del Boletín (${activeQuarter})`}</h3>
                  ...
                  <div className="flex-1">
                    <div className="text-center mb-8">
                      ...
                      <h1 className="text-2xl font-black uppercase tracking-wider" style={{ color: '#193D6D' }}>
                        {`BOLETÍN ACADÉMICO - ${activeQuarter}`}
                      </h1>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8 p-4 bg-slate-50 border border-slate-200 rounded-xl print:bg-transparent print:border-none print:p-0">
                      ...
                      <div className="text-right print:text-left">
                        ...
                        <p className="font-black text-lg text-slate-800">
                          {`${child?.us_grade_level || 'N/A'} • ${ACADEMIC_YEAR}`}
                        </p>
                      </div>
                    </div>
                    ...
                    {sortedSubjects.length === 0 ? (
                      <div className="text-center py-12 border-2 border-dashed border-slate-200 rounded-xl">
                        ...
                        <p className="text-sm text-slate-400 mt-1">
                          {`No hay materias registradas para ${activeQuarter} del periodo ${ACADEMIC_YEAR}.`}
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-6">
      ...
      {subjectSelectionStep && (() => {
        const availableSubjects = getChildSubjects(selectedChildId, activeQuarter);
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
              ...
              <div className="p-6 space-y-3 max-h-[60vh] overflow-y-auto">
                {availableSubjects.length === 0 ? (
                  <p className="text-center text-slate-500 py-8">{`No hay materias disponibles para ${activeQuarter}.`}</p>
                ) : (
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
              <div className="p-5 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                <h3 className="font-black text-lg text-slate-800">
                  Notas Parciales: {selectedStudentSubjectForEntries.subject_name}
                </h3>
                <button
              ...
              <div className="p-6 overflow-y-auto flex-1">
                <GradeEntriesManager
                  ...
                  onEntriesChange={(average) => {
                    setStudentSubjects((prev) => prev.map((subject) => subject.id === selectedStudentSubjectForEntries.id ? { ...subject, grade: average } : subject));
                    setSelectedStudentSubjectForEntries((prev) => prev ? { ...prev, grade: average } : prev);
                  }}
                />
              </div>
              <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end">
